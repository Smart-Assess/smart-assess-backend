...
async def join_course(
    ...
    course_teacher = db.query(Teacher).filter(Teacher.id == course.teacher_id).first()
    if course_teacher.university_id != current_student.university_id:
        raise HTTPException(
            status_code=403, detail="You can only join courses from your university"
        )
    ...
    join_request = StudentCourse(
        student_id=current_student.id, course_id=course.id, status="pending"
    )
...
async def submit_assignment(
    ...
    if file_size > MAX_FILE_SIZE:
        raise HTTPException(
            status_code=413, detail=f"File size exceeds the limit of 8MB"
        )
    ...
    if existing_submission:
        ...
        if not delete_success:
            print(
                f"Failed to delete old submission: {existing_submission.submission_pdf_url}"
            )
...
@router.put(
    "/student/assignment/{assignment_id}/update-submission", response_model=dict
)
async def update_assignment_submission(
    ...
    try:
        ...
        if not delete_success:
            print(
                f"Warning: Failed to delete old submission from S3: {existing_submission.submission_pdf_url}"
            )
        ...
        existing_evaluation = (
            ...
            .filter(AssignmentEvaluation.submission_id == existing_submission.id)
            .first()
        )
        ...
        db.rollback()
        raise HTTPException(
            status_code=500, detail=f"Failed to update submission: {str(e)}"
        )
...
async def get_enrolled_courses(
    db: Session = Depends(get_db), current_student: Student = Depends(get_current_admin)
):
    enrolled_courses = (
        db.query(Course)
        .join(StudentCourse)
    ...
    return {"success": True, "status": 200, "courses": courses_data}
...
async def get_course_materials(
    ...
    return {"success": True, "status": 200, "course_materials": course.pdf_urls}
...
async def get_student_results(
    ...
    submissions = (
        db.query(AssignmentSubmission)
        .filter(AssignmentSubmission.student_id == current_student.id)
        .all()
    )
    ...
    try:
        for submission in submissions:
            ...
            for evaluation in evaluations:
                    ...
                    detailed_questions.append(
                        {
                            ...
                            "plagiarism_score": scores.get("plagiarism", {}).get(
                                "score", 0
                            ),
                ...
                if total_score == 0 and any(
                    q["plagiarism_score"] > 0.7 for q in detailed_questions
                ):
                ...
                results_data.append(
                    {
                        ...
                        "feedback": feedback,
                        "evaluated_at": evaluation.created_at.strftime(
                            "%Y-%m-%d %H:%M"
                        ),
                    }
                )
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch student results: {str(e)}"
        )

    return {"success": True, "status": 200, "results": results_data}
...
async def get_assignment_result(
    ...
    if not submission:
        raise HTTPException(
            status_code=404, detail="No submission found for this assignment"
        )
    ...
    try:
        ...
        if not evaluation_data:
            raise HTTPException(
                status_code=404, detail="Detailed evaluation results not found"
            )
        ...
        total_score = (
            overall_scores.get("total", {}).get("score", 0) if overall_scores else 0
        )
        ...
        result_data = {
            ...
            "scores": {
                "plagiarism": (
                    overall_scores.get("plagiarism", {}).get("score", 0)
                    if overall_scores
                    else (evaluation.plagiarism_score if evaluation else None)
                ),
                "ai_detection": (
                    overall_scores.get("ai_detection", {}).get("score", 0)
                    if overall_scores
                    else (evaluation.ai_detection_score if evaluation else None)
                ),
                "grammar": (
                    overall_scores.get("grammar", {}).get("score", 0)
                    if overall_scores
                    else (evaluation.grammar_score if evaluation else None)
                ),
            },
        }
        ...
        result_data["scores"] = {
            k: v for k, v in result_data["scores"].items() if v is not None
        }
        ...
        raise HTTPException(
            status_code=500, detail=f"Failed to fetch evaluation results: {str(e)}"
        )
...
async def get_assignment_details(
    ...
    assignment = (
        db.query(Assignment).join(Course).filter(Assignment.id == assignment_id).first()
    )
