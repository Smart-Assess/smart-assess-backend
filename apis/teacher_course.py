    ...
    if files:
        ...
        for file in files:
            ...
            if ext not in ["pdf", "ppt", "pptx"]:
                raise HTTPException(
                    status_code=400, detail=f"File {file.filename} must be a PDF or PPT"
                )
...
async def get_teacher_courses(
    ...
    total = db.query(Course).filter(Course.teacher_id == current_teacher.id).count()
...
async def update_course(
    ...
    try:
        ...
        existing_filenames = {
            url.split("/")[-1].split("_", 1)[-1] for url in existing_urls
        }
                ...
                if url in updated_urls:
                    if delete_from_s3(url):
                        try:
                            ...
                            print(
                                f"Warning: Failed to delete embeddings for {url}: {e}"
                            )
        ...
        if pdfs:
            ...
            for pdf in pdfs:
                ...
                try:
                    ...
                    pdf_url = upload_to_s3(
                        folder_name=folder_name, file_name=s3_key, file_path=file_path
                    )
                    ...
                    if pdf_url:
                        try:
                            ...
                            print(
                                f"Warning: Failed to store embeddings for {pdf_url}: {e}"
                            )
                        updated_urls.append(pdf_url)
                        existing_filenames.add(filename)
                    else:
                        raise HTTPException(
                            status_code=500, detail=f"Failed to upload PDF {filename}"
                        )
                finally:
...
async def get_course_requests(
    ...
    requests = (
        ...
        .join(Student)
        .filter(StudentCourse.course_id == course_id, StudentCourse.status == "pending")
        .all()
    )
