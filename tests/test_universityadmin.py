...
async def test_add_student_image_upload_failure(monkeypatch):
    ...
    # Mock os functions
    with patch("os.path.join", return_value="temp/test.jpg"), patch(
        "os.makedirs"
    ), patch("builtins.open", mock_open := MagicMock()), patch("os.remove"), patch(
        "apis.universityadmin.upload_to_s3", return_value=None
    ):
...
async def test_get_student_not_found(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_student(
            student_id="2023-123-CS", db=mock_db, current_admin=mock_current_admin
        )
...
async def test_update_student_image_upload_failure(monkeypatch):
    ...
    with patch("apis.universityadmin.delete_from_s3", return_value=True), patch(
        "apis.universityadmin.send_email"
    ), patch("apis.universityadmin.Form", lambda x=None: x), patch(
        "apis.universityadmin.File", lambda x=None: x
    ):
        ...
        with patch("builtins.open", MagicMock()), patch(
            "apis.universityadmin.upload_to_s3",
            ...
            assert exc_info.value.status_code == 500
            assert (
                "An error occurred while processing the image" in exc_info.value.detail
            )
...
async def test_get_teacher_not_found(monkeypatch):
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_teacher(
            teacher_id="CS-123", db=mock_db, current_admin=mock_current_admin
        )
...
async def test_update_teacher_image_upload_failure(monkeypatch):
    ...
    mock_db.query.side_effect = [
        MagicMock(
            filter=MagicMock(
                return_value=MagicMock(first=MagicMock(return_value=mock_teacher))
            )
        ),
        query_mock,
    ]
