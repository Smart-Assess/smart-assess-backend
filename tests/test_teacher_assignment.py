...
async def test_get_teacher_assignments_db_error():
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_teacher_assignments(db=mock_db, current_teacher=mock_current_teacher)
...
async def test_get_course_assignments_course_not_found():
    ...
    with pytest.raises(HTTPException) as exc_info:
        await get_course_assignments(
            course_id=999, db=mock_db, current_teacher=mock_current_teacher
        )
...
async def test_create_assignment_course_not_found():
    ...
    with patch("apis.teacher_assigment.Form", lambda x=None: x), patch(
        "apis.teacher_assigment.File", lambda x=None: x
    ):
...
async def test_create_assignment_invalid_qa_format():
    ...
    with patch("apis.teacher_assigment.Form", lambda x=None: x), patch(
        "apis.teacher_assigment.File", lambda x=None: x
    ), patch("builtins.open", MagicMock()), patch("os.makedirs"), patch(
        "apis.teacher_assigment.PDFQuestionAnswerExtractor"
    ) as mock_extractor:
        ...
        # Empty dict (no questions found)
        extractor_instance.parse_qa.return_value = {}
...
async def test_create_assignment_s3_upload_failure():
    ...
    with patch("apis.teacher_assigment.Form", lambda x=None: x), patch(
        ...
        "apis.teacher_assigment.sanitize_folder_name", return_value="test_course"
    ), patch(
        "apis.teacher_assigment.upload_to_s3", return_value=None
    ):  # S3 upload fails
...
async def test_update_assignment_invalid_pdf_type():
    ...
    with patch(
        "apis.teacher_assigment.update_assignment.__defaults__",
...
async def test_delete_assignment_db_error():
    ...
    mock_assignment = MagicMock(
        id=1, course_id=1, question_pdf_url="https://example.com/test.pdf"
    )
    mock_db = MagicMock()
    # Course and assignment exist
    mock_db.query.return_value.filter.return_value.first.side_effect = [
    ...
    # Mock MongoDB and S3
    with patch("apis.teacher_assigment.db_mongo") as mock_mongo, patch(
        "apis.teacher_assigment.delete_from_s3", return_value=True
    ):
