...
async def test_get_university_not_found(mocker):
    ...
    # Execute and Assert
    with pytest.raises(HTTPException) as exc_info:
        await get_university(university_id=999, db=mock_db, current_admin=mock_current_admin)
...
async def test_get_university_admin_not_found(mocker):
    ...
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_university, None]
    mock_current_admin = mocker.MagicMock()
    
    
    # Execute and Assert
    with pytest.raises(HTTPException) as exc_info:
        await get_university(university_id=1, db=mock_db, current_admin=mock_current_admin)
...
async def test_delete_university_not_found(mocker):
    ...
    # Execute and Assert
    with pytest.raises(HTTPException) as exc_info:
        await delete_university(university_id=999, db=mock_db, current_admin=mock_current_admin)
...
async def test_delete_university_db_error(mocker):
    ...
    # Execute and Assert
    with pytest.raises(HTTPException) as exc_info:
        await delete_university(university_id=1, db=mock_db, current_admin=mock_current_admin)
...
async def test_update_university_email_taken(mocker):
    ...
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_university, mock_existing_university, None]
    mock_current_admin = mocker.MagicMock()
...
async def test_update_university_admin_not_found(mocker):
    ...
    mock_db = mocker.MagicMock()
    # University exists but admin doesn't
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_university, None, None]
    mock_current_admin = mocker.MagicMock()
...
async def test_update_university_admin_email_taken(mocker):
    ...
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_university, None, mock_university_admin, mock_existing_admin]
    mock_current_admin = mocker.MagicMock()
...
async def test_update_university_image_upload_failure(mocker):
    ...
    mock_db = mocker.MagicMock()
    mock_db.query.return_value.filter.return_value.first.side_effect = [mock_university, None, mock_university_admin, None]
    mock_current_admin = mocker.MagicMock()
    ...
    mocker.patch("os.path.join", return_value="/tmp/test.jpg")  # Match the path in the implementation
    mocker.patch("builtins.open", mocker.mock_open())
    
    # Mock S3 upload to raise an exception instead of returning None
    mocker.patch("apis.superadmin.upload_to_s3", side_effect=Exception("S3 upload failed"))
