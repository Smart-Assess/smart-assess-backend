    ...
    async def wrapper(*args, **kwargs):
        ...
        for param_name, param in original_sig.parameters.items():
            if param_name in kwargs and param.default is not param.empty:
                ...
                ) and param.default.__class__.__name__ in ["Form", "File"]:
                    # Replace with the inner value of the Form or File object
                    if (
                        param.default.default is not ...
                    ):  # ... is Ellipsis, used for required fields
...
async def test_update_course_s3_upload_failure():
    ...
    mock_course = MagicMock(
        id=1, teacher_id=1, pdf_urls=json.dumps([]), collection_name="test_collection"
    )
    mock_db = MagicMock()
    ...
    with patch("apis.teacher_course.Form", lambda x=None: x), patch(
        "apis.teacher_course.File", lambda x=None: x
    ):

        # Mock file and S3 operations
        with patch(
            "tempfile.NamedTemporaryFile", return_value=MagicMock(name="temp.pdf")
        ), patch(
            "apis.teacher_course.get_teacher_rag", return_value=MagicMock()
        ), patch(
            "os.path.exists", return_value=True
        ), patch(
            "os.remove"
        ), patch(
            "apis.teacher_course.upload_to_s3", return_value=None
...
async def test_delete_course_db_error():
    ...
    with patch("apis.teacher_course.delete_from_s3", return_value=True), patch(
        "apis.teacher_course.get_teacher_rag", return_value=MagicMock()
    ):
        ...
        with pytest.raises(HTTPException) as exc_info:
            await delete_course(
                course_id=1, db=mock_db, current_teacher=mock_current_teacher
            )
