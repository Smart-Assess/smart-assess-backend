...
flags.DEFINE_string(
    "candidate_checkpoint", None, "Path to BLEURT bleurt_checkpoint to benchmark."
)
...
flags.DEFINE_string(
    "test_file", None, "Path to JSONL ratings file to be used as test data."
)

flags.DEFINE_string(
    "results_json", None, "JSON file where the results will be written."
)

flags.DEFINE_integer(
    "sample_size", None, "Samples N items from the human ratings without replacement."
)
...
def grouped_wmt_kendall(df, year=2019, threshold=25):
    ...
    tf.logging.debug("Found {} agreements among {} pairs".format(agreement, n_pairs))
    ...
    for i in range(n_items - 1):
        for j in range(i + 1, n_items):
            ...
            if (
                ...
                and abs(raw_ratings[i] - raw_ratings[j]) < threshold
            ):
                n_skipped += 1
                continue
            ...
            if raw_ratings[i] < raw_ratings[j]:
                ...
                raise ValueError(
                    "Wrong raw_ratings values: {}, {}".format(
                        raw_ratings[i], raw_ratings[j]
                    )
                )
            ...
            if predictions[i] < predictions[j]:
                ...
                raise ValueError(
                    "Wrong prediction values: {}, {}".format(
...
def eval_checkpoint(export_dir, test_file, results_json=None):
    ...
    def _scoring_fun(test_df):
        ...
        return scorer.score(
            references=test_df.reference.tolist(), candidates=test_df.candidate.tolist()
        )
...
def eval_prediction_file(
    prediction_file, test_file, results_json=None, wmt_format=True, exclude_sys=None
):
    ...
    def _predict_from_file(test_data_df, filter_newstest=True):
        ...
        join_df = test_data_df.merge(
            predictions_df, how="left", on=["system", "segment_id", "lang"], sort=False
        )
...
def run_eval(scoring_fun, test_file, results_json=None, exclude_sys=None):
    ...
    for col in ["year", "lang", "reference", "candidate", "score"]:
        assert col in test_df.columns, "Field {} not found".format(col)
    ...
    for group_name, group_df in grouped_by_lang:
        ...
        grouped_by_system = (
            group_df.groupby("system")
            .agg({"bleurt": "mean", "score": "mean"})
            .reset_index()
        )
        grouped_by_system = grouped_by_system[np.isfinite(grouped_by_system["score"])]
        ...
        grouped_by_system_nooutl = grouped_by_system[
            ~grouped_by_system["system"].isin(outliers)
        ]
...
def main(_):
    if FLAGS.candidate_checkpoint:
        eval_checkpoint(FLAGS.candidate_checkpoint, FLAGS.test_file, FLAGS.results_json)
    if FLAGS.candidate_predictions_file:
        eval_prediction_file(
            FLAGS.candidate_predictions_file, FLAGS.test_file, FLAGS.results_json
        )
