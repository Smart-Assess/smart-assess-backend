...
flags.DEFINE_string(
    "data_dir",
    "/tmp/wmt_data",
    "Directory where the train, dev, and test sets will be " "stored.",
)
...
def run_benchmark():
    ...
    db_builder.create_wmt_dataset(
        train_ratings_file, FLAGS.train_years, FLAGS.target_language
    )
    ...
    logging.info("\n*** Creating test data. ***")
    db_builder.create_wmt_dataset(
        test_ratings_file, FLAGS.test_years, FLAGS.target_language
    )
