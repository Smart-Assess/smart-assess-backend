...
flags.DEFINE_string(
    "target_file", "/tmp/wmt_eval.jsonl", "Path to JSONL ratings file to be created."
)
...
flags.DEFINE_string(
    "temp_directory",
    None,
    "[optional] Temporary directory where WMT archives "
    "will be downladed and untared.",
)

# Post-processing.
flags.DEFINE_float(
    "dev_ratio", None, "Ratio of data allocated to dev set. No split if None"
)
...
def create_wmt_dataset(target_file, rating_years, target_language):
    ...
    assert (
        target_language == "*" or len(target_language) == 2
    ...
    with tempfile.TemporaryDirectory(dir=FLAGS.temp_directory) as tmpdir:
        ...
        for year in rating_years:
            ...
            for lang_pair in lang_pairs:
                ...
                logging.info(
                    "Generating records for {} and language pair {}".format(
                        year, lang_pair
                    )
                )
...
def postprocess(target_file, remove_null_refs=True, average_duplicates=True):
    ...
    if average_duplicates:
        ...
        ratings_df = (
            ratings_df.groupby(by=["lang", "candidate", "reference"])
            .agg(
                {
                    "source": "first",
                    "year": "first",
...
def _shuffle_no_leak(all_ratings_df, n_train):
    ...
    def is_split_leaky(ix):
        return (
            all_ratings_df.iloc[ix].reference == all_ratings_df.iloc[ix - 1].reference
        )
    ...
    if n_dev_sentences == 1 and is_split_leaky(split_ix):
        raise ValueError(
            "Failed splitting data--not enough distinct dev sentences"
            " to prevent leak."
        )
    ...
    if n_train != split_ix:
        logging.info(
            "Moved split point from {} to {} to prevent "
    ...
    tf.logging.info(
        "Using {} and {} unique sentences for train and dev.".format(
...
def shuffle_split(
    ratings_file, train_file=None, dev_file=None, dev_ratio=0.1, prevent_leaks=True
):
    ...
    logging.info(
        "Will attempt to set aside {} out of {} rows for dev.".format(
    ...
    logging.info(
        "Created train and dev files with {} and {} records.".format(
...
def main(_):
    create_wmt_dataset(FLAGS.target_file, FLAGS.rating_years, FLAGS.target_language)
