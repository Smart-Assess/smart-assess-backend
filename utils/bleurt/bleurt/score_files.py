...
flags.DEFINE_string(
    "candidate_file",
    None,
    "Path to candidates text file, with one candidate sentence " "per line.",
)

flags.DEFINE_string(
    "reference_file",
    None,
    "Path to reference text file, with one reference sentence " "per line.",
)
...
flags.DEFINE_string("bleurt_checkpoint", None, "[optional] Path to BLEURT checkpoint.")

flags.DEFINE_integer("bleurt_batch_size", 16, "Number of sentence pairs per batch.")
...
flags.DEFINE_bool(
    "batch_same_length",
    False,
    "Enables dynamic batching to speed up inference." " [experimental feature]",
)
...
def _json_generator(sentence_pairs_file):
    ...
    assert tf.io.gfile.exists(
        sentence_pairs_file
    ), "Sentence pairs file {} not found".format(sentence_pairs_file)
...
def _text_generator(reference_file, candidate_file):
    ...
    assert tf.io.gfile.exists(reference_file), "Reference file {} not found".format(
        reference_file
    )
    assert tf.io.gfile.exists(candidate_file), "Candidate file {} not found".format(
        candidate_file
    )
...
def score_files(generator, bleurt_checkpoint):
    ...
    if not FLAGS.batch_same_length:
        ...
        logging.warning(
            "Enabling same length batching. BEWARE: this is an experimental " "feature."
        )
...
def check_flags_and_score():
    ...
    if FLAGS.sentence_pairs_file:
        ...
        sentence_pairs_generator = _text_generator(
