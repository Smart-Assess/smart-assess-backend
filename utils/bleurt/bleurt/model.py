...
# BLEURT flags.
flags.DEFINE_string(
    "bleurt_checkpoint_name", "bert_custom", "Name of the BLEURT export to be created."
)

flags.DEFINE_string(
    "init_bleurt_checkpoint", None, "Existing BLEURT export to be fine-tuned."
)
...
flags.DEFINE_bool(
    "dynamic_seq_length", True, "Exports model with dymaic sequence length."
)
...
flags.DEFINE_integer(
    "shuffle_buffer_size", 500, "Size of buffer used to shuffle the examples."
)

# Flags to contol optimization.
flags.DEFINE_enum(
    "optimizer", "adam", ["adam", "sgd", "adagrad"], "Which optimizer to use."
)
...
# BLEURT model flags.
flags.DEFINE_integer(
    "n_hidden_layers", 0, "Number of fully connected/RNN layers before prediction."
)

flags.DEFINE_integer("hidden_layers_width", 128, "Width of hidden layers.")

flags.DEFINE_float("dropout_rate", 0, "Probability of dropout over BERT embedding.")
...
def ranking_loss(predictions, labels):
    ...
    ii, jj = tf.meshgrid(
        tf.range(0, tf.shape(predictions)[0]),
        tf.range(0, tf.shape(predictions)[0]),
        indexing="ij",
    )
...
def concat_tensors(predictions, ratings, sources=None):
    ...
    concat_predictions_value, concat_predictions_update = metrics.streaming_concat(
        predictions
    )
    concat_labels_value, concat_labels_update = metrics.streaming_concat(ratings)
    if sources is None:
        return (
            concat_predictions_value,
            concat_labels_value,
            tf.group(concat_predictions_update, concat_labels_update),
        )

    concat_sources_value, concat_sources_update = metrics.streaming_concat(sources)
    return (
        concat_predictions_value,
        concat_labels_value,
        concat_sources_value,
        tf.group(
            concat_predictions_update, concat_labels_update, concat_sources_update
        ),
    )
...
def kendall_tau_metric(predictions, ratings, weights=None):
    ...
    with tf.variable_scope("kendall_tau"):
        concat_predictions_value, concat_labels_value, update_op = concat_tensors(
            predictions, ratings
        )
...
def input_fn_builder(
    tfrecord_file, seq_length, is_training, batch_size, drop_remainder=True
):
    ...
    def input_fn(params):  # pylint: disable=unused-argument
        ...
        if n_files > 1:
            logging.info(
                "Found {} files matching {}".format(str(n_files), tfrecord_file)
            )
...
def _serving_input_fn_builder(seq_length):
    ...
    return tf_estimator.export.build_raw_serving_input_receiver_fn(name_to_features)
...
def run_finetuning(
    ...
    logging.info(
        "Train batch size: {}, eval batch size: {}".format(
    ...
    if isinstance(dev_tfrecord, str):
        ...
        for i in range(1, len(dev_tfrecord)):
            ...
            eval_name = (
                multi_eval_names[i]
                if multi_eval_names and len(multi_eval_names) > i
                else "eval_%s" % i
            )
