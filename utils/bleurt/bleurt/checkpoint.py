...
def get_bleurt_params_from_flags_or_ckpt():
    ...
    if FLAGS.init_bleurt_checkpoint:
        logging.info(
            "Reading paramter from BLEURT checkpoint: {}".format(
        ...
        if FLAGS.max_seq_length:
            logging.warning(
                "Overriding `max_seq_length`. This could have unintended"
                " consequences."
            )
        ...
        logging.info("Reads parameters from flags.")
        assert (
            FLAGS.vocab_file and FLAGS.do_lower_case is not None
        ) or FLAGS.sentence_piece_model, (
...
def finalize_bleurt_checkpoint(tf_export_path):
    ...
    if bleurt_params["vocab_file"]:
        ...
        tf.io.gfile.copy(bleurt_params["vocab_file"], vocab_copy_loc, overwrite=True)
        vocab_val = "vocab.txt"
