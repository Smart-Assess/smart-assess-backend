...
def serialize_example(reference, candidate, tokenizer, max_seq_length, score=None):
    ...
    input_ids, input_mask, segment_ids = encode_example(
        reference, candidate, tokenizer, max_seq_length
    )
...
def encode_and_serialize(
    input_file, output_file, vocab_file, do_lower_case, sp_model, max_seq_length
):
    ...
    for col in ["reference", "candidate", "score"]:
        assert col in examples_df.columns, "field {} not found in input file!".format(
            col
        )
