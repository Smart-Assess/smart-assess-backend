...
class AssignmentScoreCalculator:
    def __init__(self, total_grade: float, num_questions: int, db: Session):
        ...
        self.plagiarism_threshold = (
            0.9  # If plagiarism score is above 90%, zero the score
        )
        self.ai_threshold = 0.9  # If AI score is above 90%, zero the score

    def calculate_question_score(
        self, context_score, plagiarism_score=None, ai_score=None, grammar_score=None
    ):
        ...
        try:
            ...
            plagiarism = (
                float(plagiarism_score) if plagiarism_score is not None else 0.0
            )
            ...
            plagiarism_penalty = min(
                plagiarism * self.plagiarism_penalty, self.max_penalty
            )
            ai_penalty = min(ai * self.ai_detection_penalty, self.max_penalty)
            grammar_penalty = min(
                (1.0 - grammar) * self.grammar_penalty, self.max_penalty
            )

            # Total penalty cannot exceed max_penalty
            total_penalty = min(
                plagiarism_penalty + ai_penalty + grammar_penalty, self.max_penalty
            )
            ...
            print(
                f"Scores: context={context_score}, plagiarism={plagiarism_score}, ai={ai_score}, grammar={grammar_score}"
            )
            return 0.0

    def calculate_submission_evaluation(
        self, question_results: Dict[str, Dict[str, float]]
    ) -> Dict:
