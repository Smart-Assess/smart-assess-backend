    ...
    def run(self, pdf_files, total_grade, submission_ids=None):
        ...
        if self.request.enable_ai_detection:
            try:
                ...
                ai_delay = getattr(self.request, "ai_detection_delay", 0.3)  # Reduced from 0.5
        ...
        if self.request.enable_grammar:
            ...
            try:
                ...
                grammar_delay = getattr(self.request, "grammar_delay", 0.1)  # Reduced from 0.2
                ...
                print(f"📊 Processing {len(questions_answers_by_submission)} submissions for grammar")
                ...
                for submission_id, qa_pairs in questions_answers_by_submission.items():
                    ...
                    print(f"📝 Found {len(answers_to_check)} answers to check for submission {submission_id}")
                    
                    # Batch process all answers for this submission
                    grammar_results = self.grammar_checker.evaluate_batch(answers_to_check, grammar_delay)
                    print(f"📝 Grammar results for submission {submission_id}: {grammar_results}")
                ...
                if overall_grammar_updates:
                    mongo_db.db['evaluation_results'].bulk_write(overall_grammar_updates)
            ...
            if i < len(submission_ids):
                ...
                if eval_doc:
                    ...
                    for question in questions:
                        ...
                        print(f"Processing question {q_num}, scores type: {type(scores)}, scores: {scores}")
                        
                        # Safely extract scores, handling both dict and float formats
                        try:
                            context_score = scores.get("context", {}) if isinstance(scores, dict) else {}
                            print(f"Context score raw: {context_score}, type: {type(context_score)}")
                            
                            if isinstance(context_score, dict):
                                context_score = context_score.get("score", 0)
                            else:
                                context_score = float(context_score) if context_score is not None else 0
                                
                            plagiarism_score = scores.get("plagiarism", {}) if isinstance(scores, dict) else {}
                            if isinstance(plagiarism_score, dict):
                                plagiarism_score = plagiarism_score.get("score", 0)
                            else:
                                plagiarism_score = float(plagiarism_score) if plagiarism_score is not None else 0
                                
                            ai_score = scores.get("ai_detection", {}) if isinstance(scores, dict) else {}
                            if isinstance(ai_score, dict):
                                ai_score = ai_score.get("score", 0)
                            else:
                                ai_score = float(ai_score) if ai_score is not None else 0
                                
                            grammar_score = scores.get("grammar", {}) if isinstance(scores, dict) else {}
                            if isinstance(grammar_score, dict):
                                grammar_score = grammar_score.get("score", 0)
                            else:
                                grammar_score = float(grammar_score) if grammar_score is not None else 0
                            
                            print(f"Extracted scores - context: {context_score}, plagiarism: {plagiarism_score}, ai: {ai_score}, grammar: {grammar_score}")
                            ...
                            logger.error(f"Error processing scores for question {q_num}: {e}")
                                    ...
                                    },
                                    {
                                        "$set": {
                                            "questions.$.scores.total": {
                                                "score": round(question_results[q_key]["total_score"], 4),
                                                "evaluated_at": datetime.now(timezone.utc)
                                            }
                                        }
                                    }
                    ...
                    if question_score_updates:
                        mongo_db.db['evaluation_results'].bulk_write(question_score_updates)
