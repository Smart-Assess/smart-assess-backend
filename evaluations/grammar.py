...
class GrammarChecker:
    ...
    def query_api(self, text, attempt=0, delay=0):
        ...
        if attempt >= self.max_retries:
            logger.warning(
                "Maximum retries reached for grammar API, using default score"
            )
    ...
    def evaluate_batch(self, texts_dict, delay=0):
        ...
        for key, text in texts_dict.items():
            ...
            if not self.service_available:
                ...
                logger.info(
                    f"Service unavailable, using simulated score for {key}: {simulated_score}"
                )
    ...
    def evaluate(self, text, delay=0):
        ...
        if not self.service_available:
            simulated_score = round(random.uniform(0.8, 1.0), 4)
            logger.info(
                f"Grammar service unavailable, using simulated score: {simulated_score}"
            )
        ...
        weighted_score = (
            ...
            if total_length > 0
            else sum(all_scores) / len(all_scores)
        )
