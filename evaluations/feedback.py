...
class FeedbackGenerator:
    def __init__(self, course_id: int, assignment_id: int):
        ...
        self.groq_api_key = os.getenv(
            "GROQ_API_KEY", "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"
        )
...
Target 1-2 key areas for improvement based on the weakest scores."""
    ...
    def generate_question_feedback(
        ...
        formatted_prompt = self.question_prompt.format(
            ...
            context_score=f"{scores.get('context', {}).get('score', 0) * 100:.2f}%",
                ...
                try:
                    response = self.client.chat.completions.create(
                        messages=[
                            {
                                "role": "system",
                                "content": "You are a helpful educational assistant providing very brief, actionable feedback. Limit your response to 2-3 sentences maximum.",
                            },
                    ...
                    print(
                        f"Error calling Groq API (attempt {attempt+1}/{max_retries}): {str(e)}"
                    )
    ...
    def generate_overall_feedback(
        ...
        question_feedback: Dict[int, str],
        delay: float = None,
    ) -> str:
    ...
    Based on the individual question feedback and scores above, provide overall assessment and 1-2 key areas for improvement. Be specific and constructive."""
        ...
        try:
                ...
                try:
                    response = self.client.chat.completions.create(
                        messages=[
                            {
                                "role": "system",
                                "content": "You are a helpful educational assistant providing specific, actionable feedback. Limit your response to 2-3 sentences maximum.",
                            },
                            {"role": "user", "content": formatted_prompt},
                        ],
                        model=self.model,
            ...
            return (
                "Overall feedback generation failed. Please review the scores manually."
            )

    def save_feedback_to_mongo(
        self,
        submission_id: str,
    ...
    def run(
        ...
        print(
            f"Teacher questions sample: {list(teacher_questions.keys())[:3] if teacher_questions else 'None'}"
        )
        ...
        for i, pdf_file in enumerate(pdf_files):
            ...
            print(
                f"Student submission {submission_id} Q&A pairs: {len(qa_pairs)} items"
            )
            print(
                f"Student submission {submission_id} Q&A sample: {list(qa_pairs.keys())[:3] if qa_pairs else 'None'}"
            )
            ...
            for question in questions:
                ...
                print(
                    f"Q{q_num}: Found question text: {bool(question_text)} ({len(question_text)} chars)"
                )
                print(
                    f"Q{q_num}: Found student answer: {bool(student_answer)} ({len(student_answer)} chars)"
                )

                # Add detailed debugging for specific case when either is missing
                if not question_text:
                    print(
                        f"WARNING: Missing question text for {q_key}. Available keys: {list(teacher_questions.keys())[:5]}"
                    )
                if not student_answer:
                    print(
                        f"WARNING: Missing student answer for {a_key}. Available keys: {list(qa_pairs.keys())[:5]}"
                    )
                ...
                if not question_text or not student_answer:
                    ...
                    print(
                        f"Using fallback text: Q: '{question_text}', A: '{student_answer}'"
                    )

                feedback = self.generate_question_feedback(
                    q_num, scores, question_text, student_answer, delay
                )
            ...
            overall_feedback = self.generate_overall_feedback(
                overall_scores, question_feedback, delay
            )
            print(f"Generated overall feedback: {overall_feedback[:100]}...")

            # Save to MongoDB
            self.save_feedback_to_mongo(
                submission_id, overall_feedback, question_feedback
            )
